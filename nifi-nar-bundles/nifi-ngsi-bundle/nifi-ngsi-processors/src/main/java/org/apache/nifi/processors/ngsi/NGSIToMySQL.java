...
public class NGSIToMySQL extends AbstractSessionFactoryProcessor {
    ...
    private final GroupingFunction groupFlowFilesBySQLBatch = (context, session, fc, conn, flowFiles, groups, sqlToEnclosure, result) -> {
        for (final FlowFile flowFile : flowFiles) {
            ...
            final String tableName = mysql.buildTableName(event, context.getProperty(DATA_MODEL).getValue(), context.getProperty(ENABLE_ENCODING).asBoolean());
            final String dbName = mysql.buildDbName(event.getFiwareService(), context.getProperty(ENABLE_ENCODING).asBoolean());
