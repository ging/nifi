...
public abstract class AbstractMongoProcessor extends AbstractProcessor {
    ...
    protected void persistFlowFile(final ProcessContext context, final FlowFile flowFile,ProcessSession session) {
        ...
        final String collectionName = mongoClient.buildCollectionName(event.getFiwareServicePath(),event.getEntityId(),event.getEntityType(),event.getAttrs().get(0).getAttrName()
        ...
        if(attrPersistence.compareToIgnoreCase("row")==0){
            ...
            aggregator.aggregate(event,context.getProperty(DATA_MODEL).getValue());
