...
public class MySQLBackend {
    ...
    public String buildDbName(String service,boolean enableEncoding) {
        String dbName="";
        if (enableEncoding) {
            dbName = NGSICharsets.encodeMySQL(service);
        } else {
            dbName = NGSICharsets.encode(service, false, true);
        } // if else
    ...
    public String buildTableName(NGSIEvent event, String dataModel, boolean enableEncoding){
        String tableName="";
        String servicePath=event.getFiwareServicePath();
