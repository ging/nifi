...
public class MongoBackend {
     ...
     * @return
     */
    public String buildDbName(String fiwareService,boolean enableEncoding,String dbPrefix) {
        String dbName;

        if (enableEncoding) {
            dbName = dbPrefix + NGSICharsets.encodeMongoDBDatabase(fiwareService);
        } else {
            dbName = dbPrefix + NGSICharsets.encodeSTHDB(fiwareService);
        } // if else

        if (dbName.length() > NGSIConstants.MONGO_DB_MAX_NAMESPACE_SIZE_IN_BYTES) {
            System.out.println("Building database name '" + dbName + "' and its length is greater "
     ...
     * @return
     */
    public String buildCollectionName(String fiwareServicePath, String entity, String entityType, String attribute,boolean enableEncoding,String collectionPrefix)
             {
        ...
        if (enableEncoding) {
            ...
            switch (dataModel) {
                case "db-by-service-path":
                    collectionName = NGSICharsets.encodeSTHCollection(fiwareServicePath);
                    break;
                case "db-by-entity":
                    collectionName = NGSICharsets.encodeSTHCollection(fiwareServicePath) + "_"
        ...
        if (collectionName.getBytes().length > NGSIConstants.MONGO_DB_MAX_NAMESPACE_SIZE_IN_BYTES) {
            System.out.println("Building collection name '" + collectionName + "' and its length is "
